...
- **当 `VITE_VIDEO_ADAPTIVE_BITRATE=true` (ABR启用) 时**：
  - 此限制仅对ABR自动模式有效
  - 用户可通过画质菜单手动选择更高分辨率（如1080p）
  - 画质菜单包含"自动"选项和所有可用的固定分辨率
  - 设置为 `720` 时，ABR 自动模式最高选择 720p
  - 设置为 `0` 或不设置表示不限制

- **当 `VITE_VIDEO_ADAPTIVE_BITRATE=false` (ABR禁用) 时**：
  - 播放器默认使用此配置作为**初始固定分辨率**
  - 用户仍可通过画质菜单手动选择其他分辨率（如 1080p、480p 等）
  - 画质菜单不包含"自动"选项，只显示固定分辨率选项
  - 设置为 `720` 时，播放器初始使用 720p
  - 设置为 `0` 或不设置时，使用默认的 720p（如果可用）
  - **注意**：用户手动选择后，播放器会使用用户选择的分辨率，不会自动切换
...
**A:** 可以。无论 ABR 是否启用，用户都可以通过画质菜单手动选择任何可用的分辨率。
- **ABR 启用时** (`VITE_VIDEO_ADAPTIVE_BITRATE=true`)：
  - 画质菜单包含"自动"选项和所有可用的固定分辨率
  - "自动"模式受 `VITE_VIDEO_MAX_RESOLUTION_HEIGHT` 限制
  - 用户手动选择时不受限制（包括 1080p、4K 等）
  
- **ABR 禁用时** (`VITE_VIDEO_ADAPTIVE_BITRATE=false`)：
  - 画质菜单只包含固定分辨率选项（不含"自动"）
  - 用户可以手动选择任何可用的分辨率
  - 选择后不会自动切换，播放器保持使用用户选择的分辨率
